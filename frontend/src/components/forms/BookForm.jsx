...
import { BOOK_STATUS_VALUES, BOOK_STATUS_LABELS } from '../../constants/bookStatus';
import { BOOK_VISIBILITY_VALUES, BOOK_VISIBILITY_LABELS } from '../../constants/bookVisibility';
 ...
 */
const BookForm = ({ defaultValues, onSubmit, isSubmitting = false, submitLabel = 'Save Book' }) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="language" error={!!errors.language} {...register('language')} />
          {errors.language && <p className="text-sm text-destructive">{errors.language.message}</p>}
        </div>

        <div className="space-y-2">
          <Label htmlFor="edition">Edition</Label>
          <Input id="edition" placeholder="e.g. 2nd Edition" {...register('edition')} />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input id="numberOfPages" type="number" error={!!errors.numberOfPages} {...register('numberOfPages')} />
          {errors.numberOfPages && <p className="text-sm text-destructive">{errors.numberOfPages.message}</p>}
        </div>

        <div className="space-y-2">
          <Label htmlFor="tagsInput">Tags (comma-separated)</Label>
          <Input id="tagsInput" placeholder="e.g. fiction, classic, bestseller" {...register('tagsInput')} />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2 rounded-md border border-input p-3">
          {authors.length === 0 && <p className="text-sm text-muted-foreground">No authors available yet.</p>}
          {authors.map((author) => {
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-3 rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
          ...
          File uploads will be available here once this book is saved (coming in Milestone 3).
        </div>
      </div>

...
export const createRequest = async (studentId, payload) => {
  ...
  const duplicate = await findOverlappingRequests(bookId, requestedCollectionDate, requestedReturnDate, {
    statuses: [REQUEST_STATUS.PENDING, REQUEST_STATUS.APPROVED],
  });
  const ownDuplicate = duplicate.find((r) => r.student._id.toString() === studentId.toString());
  if (ownDuplicate) {
    throw new ApiError(
      409,
      "You already have a pending or approved request for this book covering an overlapping period",
    );
  }
...
// past requests. No Loan model exists yet (M3), so "conflicts" here
// can only be other requests — that's the honest limit of what M2 can
// see, not an oversight.
const attachReviewContext = async (request) => {
  const [overlapping, studentHistory] = await Promise.all([
    findOverlappingRequests(
      ...
      { statuses: [REQUEST_STATUS.APPROVED], excludeId: request._id },
    ),
...
export const listRequestsForStudent = async (studentId, { status } = {}) =>
  PhysicalRequest.find({ student: studentId, ...(status && { status }) })
    .populate(REQUEST_POPULATE)
    .sort({ createdAt: -1 })
    .lean();
